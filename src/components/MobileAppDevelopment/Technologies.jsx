...
import { MobileAppServices, TechDescription } from "../Data/MobileApp";
    ...
    <>
      <section className="relative flex flex-col space-y-4 items-center mt-10 sm:mt-20 xl:px-32 bg-white">
        ...
        <div className="flex flex-wrap justify-center gap-3 py-3 w-[100%]">
          {MobileAppServices?.map((menu, index) => (
            <div
              data-aos="fade-up"
              key={index}
              className="flex gap-3 justify-center items-center text-black px-4 py-2 rounded-full border-2 border-black bg-white"
            >
              <img
                src={menu.icon}
                alt={menu.title}
